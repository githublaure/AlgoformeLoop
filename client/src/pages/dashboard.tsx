...
export default function Dashboard() {
  ...
  const isPigeonne = (subscription: Subscription) =>
    subscription.usageFrequency === 'rarely_used' ||
    (subscription.isSuspect && !subscription.isActive);

  const isPigeonFlagged = (subscription: Subscription) =>
    subscription.isSuspect || isPigeonne(subscription);
  ...
  const filteredSubscriptions = subscriptions.filter((subscription) => {
    ...
    if (pigeonFilter === 'pigeon' && !isPigeonFlagged(subscription)) return false;
    if (pigeonFilter === 'not-pigeon' && isPigeonFlagged(subscription)) return false;
