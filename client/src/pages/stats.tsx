...
export default function StatsPage() {
  ...
  const monthlyChartData = useMemo(() => {
    ...
    const defaultBudget = Number(monthlyBudgetsData?.defaultBudget ?? stats?.budgetCap ?? 0);
  ...
  }, [includeLifetime, monthlyBudgetsData, stats?.budgetCap, subscriptions]);
                ...
                {monthlyBudgetsData ? (
                  <div className="mt-6 rounded-lg border border-gray-100 bg-gray-50/60 p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          {currencyFormatter.format(Number(monthlyBudgetsData.defaultBudget))}
                          ).
                        </p>
                      </div>
                      <button
                        ...
                        onClick={() =>
                          saveMonthlyBudgets.mutate({
                            budgets: monthlyBudgetsData.months.map((monthKey) => {
                              const value = monthlyBudgetDrafts[monthKey];
                              return {
                                month: monthKey,
                                amount: value === "" || value === undefined ? null : value,
                              };
                            }),
                          })
                        }
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                      {monthlyBudgetsData.months.map((monthKey) => {
                        const monthDate = new Date(`${monthKey}-01T00:00:00`);
                        const monthLabel = Number.isNaN(monthDate.getTime())
                          ? monthKey
                          : formatMonthLabel.format(monthDate);
                        return (
